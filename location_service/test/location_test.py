...
class TestLocationService(unittest.TestCase):
    def setUp(self):
        ...
        self.geo_onemap = GeoOnemap()

    def test_search_address_from_postal(self):
        response = self.geo_onemap.get_from_postal(self.postal_code)
        print(response.json())
        self.assertEqual(response.status_code, 200)

    def test_get_route(self):
        start_location_response = self.geo_onemap.get_from_postal(self.postal_code)
        end_location_response = self.geo_onemap.get_from_postal(self.postal_code_2)
